function [uPR,inerER,er]=solverWFkappa(Y,x, A, At,Params,AtA,u0,lambda)

%% set up paras
[n1,n2,L]=size(Y);%by chaoyan
gamma=Params.gamma;
c=Params.const;
...
%% build A matrix
%DtD = @(x) lap(x);
B = @(x) b_mat(x,n1,n2,c,gamma,AtA,eta); 

%% initialization
% u0=HIO_image(Nx,Ny,L,Y,A,At,AtA,HIO_iter); %by chaoyan
% u0=rand(n1,n2);
u=u0;
% [px,py] = grad(u);
% qx = zeros(size(px));
...
for oit=1:maxDCA
    ...
    for i=1:nIter
        ...
        uold=u;
        b=v+FraRecMultiLevel(q,R,Level)+gamma*FraRecMultiLevel(p,R,Level)+eta*t-d;
...
%                      pt{ki}{ji,jj} = du{ki}{ji,jj}-q{ki}{ji,jj}/gamma;
                     p{ki}{ji,jj}=wthresh(du{ki}{ji,jj}-q{ki}{ji,jj}/gamma,'s',lambda/gamma);
                end
            end
       end    
...
function bx = b_mat(x,m,n,c,gamma,AtA,eta)
xx = reshape(x,[m,n]);
